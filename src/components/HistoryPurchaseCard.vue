...
<!-- TODO: hover and detials -->
  <v-card class="my-5 pa-5 rounded-lg" elevation="8">
    ...
    <v-sheet>
      <v-container
        v-for="(product, i) in purchase.Products"
        :key="product.id"
        class="d-flex align-center py-0"
      >
        <v-img
          lazy-src="https://picsum.photos/id/11/100/60"
          :src=" product.image || `https://picsum.photos/id/${12 + i + random}/500/300`"
          max-width="100px"
          contain
          class="rounded"
        >
        ...
        <v-container>
          ...
          <v-card-subtitle class="py-2"
            >{{ product.price | moneyFilter}} x {{ product.quantity }}</v-card-subtitle
          >
        </v-container>
        <v-card-subtitle class="nowrap text-h6 font-weight-bold"
          >{{ product.price * product.quantity | moneyFilter }}</v-card-subtitle
        >
      </v-container>
    </v-sheet>
    ...
    <v-container class="d-flex justify-end">
      <v-card-title class="text-h5 font-weight-black indigo--text"
        >總金額：{{ totalAmount | moneyFilter }}</v-card-title
      >
    </v-container>
  </v-card>
</template>
